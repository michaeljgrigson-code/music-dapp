...
class Gateway {
  constructor(wallet) { this.wallet = wallet; this.jwt = null; }
  async getToken() {
    if (this.jwt && this.jwt.expMs - Date.now() > 30000) return this.jwt.token;
    const ch = await fetch(`${GATEWAY}/api/auth/challenge?address=${this.wallet.address}`,
  ...
  async waitForRelayToken(sessionId, timeoutMs = 30000) {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      const t = await r.text();
      if (r.status === 200) { const p = JSON.parse(t); if (p?.token) return p.token; }
...
async function generateMusic(lyrics, genre, artist, language) {
  ...
  try {
    ...
    if (!generationId) { console.error('❌ No generation ID'); return null; }
  } catch (err) { console.error('❌ Music submit crashed:', err.message); return null; }
  ...
  for (let i = 0; i < 36; i++) {
    ...
    } catch (err) { console.error(`❌ Poll ${i + 1} crashed:`, err.message); }
  }
  return null;
}
...
async function runLyricsJob(prompt) {
  ...
  const tx1 = await reg.createSession(
    model.id, prep.worker, hexlify(encWorker), hexlify(encDisputer),
    prep.signature, BigInt(prep.expiry), { gasLimit: 1000000n }
  );
  ...
  await new Promise((resolve, reject) => {
    ...
    ws.on('message', (data) => {
      ...
      try { chunks.push(aesDecrypt(sessionKey, Buffer.from(frame.payload, 'base64')).toString('utf8').normalize('NFC')); } catch {}
    });
    ws.on('error', reject);
  });
...
function buildPrompt(prompt, genre, artist, language, mode) {
  ...
  const languageLine = language && language !== 'English'
    ? romanizedLanguages.includes(language)
      ? `Write in ${language} using romanized/latin script only (e.g. pinyin for Chinese, romaji for Japanese). Do not use any non-latin characters.`
      : `Write in ${language}.`
    : '';

  const noNotes = `CRITICAL RULES:
- Output ONLY the requested content
- NO intro sentence, NO preamble, NO "here are the lyrics" type opening
...
${noNotes}`;
  ...
  if (mode === 'poem') {
    ...
    const styleNote  = genre ? `influenced by ${genre} style` : '';
    return `Write an original poem ${artistLine} ${styleNote} about: ${prompt}. ${languageLine}
${noNotes}`;
  }

  const artistLine = artist ? `in the style of ${artist}` : '';

  if (prompt.startsWith('__own__')) {
    return `Take these lyrics and restyle them ${artistLine} in the ${genre} genre, keep the meaning intact. ${languageLine}
${noNotes}
...
${prompt.replace('__own__', '')}`;
  ...
  return `Write original song lyrics ${artistLine} in the ${genre} genre about: ${prompt}. ${languageLine}
